...
class testHTTPRequestHandler(http.server.BaseHTTPRequestHandler):
    def main_web(self):   #La web principal, con la cual se accede a todas las demás
        html = '''<html>
            <head>
            ...
            <body style = 'background-color: red' align='center' >
                ...
                <form action = 'listWarnings' method ="get">
                    <input type = 'submit' value = 'Adevertencias'>
                        Limit = <input type = 'text' name = 'limit' >


                    </input>
                </form> Practica realizada por Gonzalo Rodriguez Esteban'''
        return html


    def drugs_web(self,list_drugs):  # Esta web contiene la lista de medicamentos
        html_drugs = '''<html>
            <head>
    ...
    def companies_web(self,list_companies):  # Esta contiene la lista de compañias
        html_companies = '''<html>
            <head>
                <title> OpenFDA Companies </title>
            </head>
                <body> Estas son las empresas recuperadas:
                    <ul>'''
    ...
    def ingredient_web(self,list_ingredient):  # Esta contiene los resultados de la búsqueda de un principio activo
    ...
    def warning_web(self,list_warning):   # Esta web nos devuelve las advertencias
        html_warning = '''<html>
            <head>
    ...
    def error_web(self):  # Finalmente, esta es la web que aparecerá si los valores introducidos al hacer una búsqueda no existen
        error = '''<html>
            <head>
                <title> Error </title>
            </head>
            <body>
                <h1> Error found </h1>
                No hemos encontrado informacion sobre el valor introducido
            </body>
            </html>'''
    ...
    def do_GET(self): # Cada vez que hay una petición GET, se invoca esta funcion. El recurso solicitado esta en self.path
        ...
        elif 'listDrugs' in self.path: # listDrugs nos devuelve los mediamentos
            self.send_response(200)
            self.send_header('Content-type', 'text/html')
            self.end_headers()
